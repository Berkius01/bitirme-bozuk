...
network_conf_file = '/home/berk/Desktop/bitirme/bitirme/yeni_arayüz/ActivityDetectionAlgorithm/yolov4-bitirme.cfg'
network_weights = '/home/berk/Desktop/bitirme/bitirme/yeni_arayüz/ActivityDetectionAlgorithm/yolov4-bitirme_last.weights'
idx_to_labels = {
        0: labels[0],
        1: labels[1],
        2: labels[2],
...
def camera_thread_function():
    ...
    while True:
        ...
        # ----------------
def detection_function(network_model, output_label_dictionary, CONFIDENCE_THRESHOLD, NMS_THRESHOLD):
    ...
    while (True):
        ...
        try:
            ...
            network_model.setInput(img_blob)
            classes, scores, boxes = network_model.detect(frame, CONFIDENCE_THRESHOLD, NMS_THRESHOLD)
...
def DetectActivities():
    ...
    model.setInputParams(size=image.shape[:2], scale=1 / 255.0, swapRB=True)

    #camera_thread_function(model=model)

    camera_thread = Thread(target=camera_thread_function, args=())
    detection_thread = Thread(target=detection_function, args=(model,idx_to_labels
